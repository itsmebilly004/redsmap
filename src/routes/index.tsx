...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Redsmap Traders - Real-time Deriv Trading Platform" },
      {
        name: "description",
        content:
  ...
  useEffect(() => {
    ...
    fetchTicks(symbol, 500)
      .then((ticks) => {
        if (!cancelled) {
          setTickPrices((prev) => {
            // Only use historical ticks if we haven't already accumulated a lot
            if (prev.length < 500) {
...
function MobileDigitStatsRow({
  ...
  // Deriv's live digit circles never display more than ~12% on any single digit visually,
  // so cap the visual distribution at 12% to stay faithful to the real Dcircles behaviour.

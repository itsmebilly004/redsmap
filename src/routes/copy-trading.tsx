...
function CopyTrading() {
  return (
    <TopShell>
      <PageHero title="Copy Trading" subtitle="Follow top traders and automatically mirror their trades on your Deriv account.">
        <div className="grid gap-4 md:grid-cols-3">
          {TRADERS.map((t) => (
            <div key={t.name} className="rounded-lg border border-[oklch(0.92_0.005_240)] bg-white p-5 shadow-sm">
              ...
              <Button asChild size="sm" className="mt-4 w-full bg-[oklch(0.55_0.22_265)] text-white">
                <Link to="/auth" search={{ mode: "signup" }}>Copy</Link>
              </Button>
            </div>
          ))}
        </div>

...
export function DerivChart({
  ...
  const lastDigit = digitHistory.length > 0 ? digitHistory[digitHistory.length - 1] : null;
    ...
    <div className={cn("relative flex flex-col w-full h-full", className)}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 px-1">
        <Select value={symbol} onValueChange={(v) => onSymbolChange?.(v)}>
          <SelectTrigger className="w-52 glass-card text-xs sm:w-64 border-slate-200">
        ...
        <div className="flex bg-white rounded-lg border border-slate-200 p-0.5 overflow-hidden shadow-sm">
          {TIMEFRAMES.slice(0, 4).map((tf) => (
            <button
              ...
              className={cn(
                ...
                granularity === tf.value ? "bg-slate-900 text-white shadow-sm" : "bg-transparent text-slate-500 hover:bg-slate-50",
              )}
            >
              {tf.label}
            </button>
        ...
        <div className="flex bg-white rounded-lg border border-slate-200 p-0.5 overflow-hidden shadow-sm">
          <button type="button" onClick={() => setChartType("area")} className={cn("px-3 py-1.5 text-xs font-bold rounded-md transition", chartType === "area" ? "bg-slate-900 text-white" : "text-slate-500")}>Area</button>
          <button type="button" onClick={() => setChartType("candle")} className={cn("px-3 py-1.5 text-xs font-bold rounded-md transition", chartType === "candle" ? "bg-slate-900 text-white")}>Candle</button>
        </div>
      ...
      <div className="relative flex-1 min-h-0 bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm group">
        ...
        {isDigitTrade && (
          <div className="absolute bottom-6 left-0 right-0 flex justify-center pointer-events-none z-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex items-center gap-3 sm:gap-5 bg-white/80 backdrop-blur-md border border-white/50 p-4 rounded-[28px] shadow-2xl pointer-events-auto">
              {stats.map((s) => (
                <div key={s.digit} className="flex flex-col items-center group relative">
                  <div className="relative">
                    ...
                    {/* Freq indicator bar at bottom of the circle */}
                    <div className={cn(
      ...
      <div className="mt-3 flex items-center justify-between px-2 pb-1">
        <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
          <Info className="size-3 text-emerald-500" /> Stochastic analysis active (Last 1k ticks)
        </div>
        <div className="text-[10px] font-black text-slate-300 uppercase tracking-tighter">
          ArkTrader Hub Analytics v2.5
        </div>
      </div>
    </div>
  );
